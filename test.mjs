// Track sanity check: catch a bad track before it breaks the game.
//   node test.mjs
...
for (const f of files) {
  ...
  let t;
  try {
    t = (await import(new URL(f, dir))).default;
  } catch (e) {
    console.log(`x ${f}: won't import - ${e.message}`);
    fails++;
    continue;
  }

  if (!t || typeof t !== 'object') errs.push('default export is not an object');
  ...
  if (errs.length) {
    ...
    console.log(`ok ${f}  (${t.blocks.length} blocks)`);
  }
}
